...
public class BSTElementNode<T extends Comparable<T>> implements BSTNode<T> {
  ...
  public boolean same(BSTNode<T> other) {
    ...
    if (other instanceof BSTElementNode) {
      BSTElementNode<T> otherElement = (BSTElementNode<T>) other;
      return this.data.equals(otherElement.data)
              && this.left.same(otherElement.left)
              && this.right.same(otherElement.right);
    }
    return false;
  }
}
